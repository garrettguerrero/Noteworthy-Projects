...
def identity():
    ...
    play_name = input('Player Name: ')
    alignment = input('Alignment: ')
    return(char_name, background, play_name, alignment)
...
def class_select():
    ...
    while invalid_class == True:
        ...
        if invalid_class == True:
            char_class = input('Please choose a valid class: ')
    ...
    try: 
        ...
        print('Please make sure you have downloaded all .txt files that go along with the program and are running them from the same directory.')
...
(char_class, hit_dice, HP_max, strn_prof, dex_prof, con_prof, intl_prof, wis_prof, cha_prof, skill_prof_choices) = class_select()
...
(strn, dex, con, intl, wis, cha) = ability_score_assign(stats, strn, dex, con, intl, wis, cha)

(strn_mod, dex_mod, con_mod, intl_mod, wis_mod, cha_mod) = ability_score_mod(strn, dex, con, intl, wis, cha)
